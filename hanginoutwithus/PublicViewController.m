...
@synthesize publicEventsTable;

 // The designated initializer.  Override if you create the controller programmatically and want to perform customization that is not appropriate for viewDidLoad.
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil delegate:(id)theDelegate  {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
		...
		eventMock.image = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:@"http://media.ejbdotcom.net/thumbs/small/thumbnail_1268622128.jpg"]];
...
-(IBAction)switchTab:(id)sender {
	if([delegate respondsToSelector:@selector(didToggleTabs:)]){
		[delegate didToggleTabs:@"friends"];
	}
}
